...
def peak_prominence_db(P: np.ndarray, period: int) -> float:
    """Prominence of the period-`period` temporal peak over the local baseline, in dB.

    Baseline is the median of the non-harmonic bins below Nyquist, which is robust to the
    1/f falloff and to the peak itself.
    """
    T = len(P)
    k0 = T // period
    if k0 < 1 or k0 >= T // 2:
    ...
    usable = np.arange(1, T // 2)
    harmonics = {k0 * m for m in range(1, period)} | {k0 - 1, k0, k0 + 1}
    baseline_bins = np.array([k for k in usable if k not in harmonics])
    if len(baseline_bins) == 0:
        return float("nan")

    baseline = np.median(P[baseline_bins])
    if baseline <= 0:
        return float("nan")
...
def main():
    ...
    for i, vid in enumerate(tqdm(videos, desc="videos")):
        try:
            ...
            synth_u8 = synth_u8[: args.num_frames].to(torch.uint8)

            real_u8, _, _ = io.read_video(str(vid), pts_unit="sec")
            real_u8 = real_u8[: args.num_frames].to(torch.uint8)
            ...
            for name, u8 in variants.items():
                ...
                t = preprocess_video_tensor(u8, target_size=(256, 256), num_frames=args.num_frames)
