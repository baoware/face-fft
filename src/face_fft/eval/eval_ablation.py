...
def plot_confusion_matrix(cm, arch_name):
    ...
    plt.title(f'3D-FFT Confusion Matrix: {arch_name.upper()}', pad=20, fontsize=14, fontweight='bold')
    plt.tight_layout()
...
def main():
    ...
    for arch in ARCHITECTURES:
        ...
        # Evaluate
        y_true, y_pred, y_prob = evaluate_loader(model, test_loader, DEVICE, desc=f"Eval {arch.upper()}")
    ...
    print(f"| {'Architecture':<12} | {'Params':<8} | {'Accuracy':<10} | {'Precision':<10} | {'Recall':<10} | {'F1-Score':<10} | {'ROC-AUC':<10} |")
    print("|--------------|----------|------------|-----------|------------|------------|------------|")
    
    for arch, r in ablation_results.items():
        print(f"| {arch.upper():<12} | {r['Params']:<8} | {r['Acc']*100:>8.2f}% | {r['Prec']:>9.4f} | {r['Rec']:>8.4f} | {r['F1']:>8.4f} | {r['AUC']:>8.4f} |")
