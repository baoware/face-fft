...
def evaluate_subset(synth_models, data_root, model, device, batch_size=4):
    ...
    with torch.no_grad():
        for inputs, labels in tqdm(
            test_loader,
            desc=f"Eval {synth_models[0] if len(synth_models)==1 else 'ALL'}",
            leave=False,
        ):
...
def main():
    ...
    print(
        f"{'Generator Model':<25} | {'Accuracy':<10} | {'F1-Score':<10} | {'ROC-AUC':<10}"
    )
    print("----------")
    ...
    for sm in SYNTH_MODELS:
        ...
        m_f1 = f1_score(yt, yp)
        m_auc = roc_auc_score(yt, yprob) if len(np.unique(yt)) > 1 else float("nan")
